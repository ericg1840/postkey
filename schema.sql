...
-- (content_ideas comes first: content_posts.idea_id references it.)
CREATE TABLE content_posts (
  ...
  -- Set when this row was made from a saved idea, so dismissing the
  -- suggestion can restore the idea. See migrations/014_content_post_idea.sql.
...
-- "Save for later" drafts, synced across a user's devices. See
-- migrations/009_post_drafts.sql. `id` is generated by the client and
-- `deleted_at` is a tombstone, so a delete on one device isn't undone by a
